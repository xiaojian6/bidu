    ...
    $(document).ready(
        KlineIns.G_HTTP_REQUEST = $.ajax({
            ...
            success: function (res) {
                counts++;
                if(res.type=='ok'){
                    window.name1=res.message.legal.name;
                    window.name2=res.message.currency.name;
                    window.coin_names=res.message.currency.name+'/'+res.message.legal.name;
                    console.log(name1,name2,coin_names)
                    ...
                    for(let i=0;i<datas.length;i++){
                        var dataList=[];
                        dataList.push(ndata(datas[i].times),Number(datas[i].start_price),Number(datas[i].highest),Number(datas[i].mminimum),Number(datas[i].end_price),Number(datas[i].number))
                        dateses.unshift(dataList)
                    }
                    // for(var j=0;j<counts;j++){
                    //     dateses.push([1539306000000, 0.32, 0.62, 0.22, 0.42, 128.20889])
                    // }
                    console.log(dateses,9999)
