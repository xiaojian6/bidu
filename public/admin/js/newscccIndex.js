layui.use(['element', 'form', 'layedit', 'laypage', 'layer'], function() {
    ...
    var showCateManage = function() {
        var index = layer.open({
            title:'新闻分类管理'
            ,type:2
            ,content: '/admin/currency/news_cate_index'
            ,area: ['800px', '600px']
            ,maxmin: true
            ,anim: 3
            ,end : function() {
                //弹窗关闭后回调，刷新主窗口的分类下拉列表
                $.get('/admin/currency/news_cate_list/', function(returnData) {
    ...
    //先判定是否已经创建分类
    $.get('/admin/currency/news_cate_list/', function(data) {
        if(data.count <= 0) {
        ...
        $.get('/admin/currency/news_cate_list/', function(data) {
            if(data.count > 0) {
                var index = layer.open({
                    title:'发布新闻'
                    ,type:2
                    ,content: '/admin/currency/news_add'
                    ,area: ['800px', '600px']
                    ,maxmin: true
                    ,anim: 3
                });
    ...
    $('.newsDiscuss').click(function() {
        ...
        var index = layer.open({
            title: '评论管理'
            ,type: 2
            ,content: '/admin/currency/news_discuss_index/' + id
            ,area: ['800px', '600px']
            ,maxmin: true
        });
        layer.full(index);
    ...
    $('.newsEdit').click(function(){
        ...
        var index = layer.open({
            title: '编辑新闻'
            ,type: 2
            ,content: '/admin/currency/news_edit/' + id
            ,area: ['800px', '600px']
            ,maxmin: true
            ,anim: 2
        });
    ...
    $('.newsDel').click(function() {
        ...
        disDelUrl = '/admin/currency/news_del/' + id;
        layer.confirm('真的确定要删除吗?',{icon: 3, title:'提示'}, function(index) {
            delExect(disDelUrl, index);
        });
        return false
        //判断是否有评论，提示是否连评论一同删除
        $.get('/admin/currency/news_discuss_list/' + id, function(discussData) {
            if(discussData.count >0 ) {
                layer.confirm('是否连同评论一起删除?', {
                    btn:['是','否', '取消'],
                    btn1: function(index) {
                        disDelUrl = '/admin/currency/news_del/' + id + '/1';
                        delExect(disDelUrl, index);
                    },
                    btn2: function(index) {
                        disDelUrl += '/admin/currency/news_del/' + id;
