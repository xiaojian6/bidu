...
class TransactionOut extends Model
{
    ...
    public static function transaction($in, $num, $user, &$wallet, $legal_id, $currency_id)
    {
        ...
        try {
            ...
            //扣除卖方的交易币
            $result = change_wallet_balance($out_currency, 2, -$num, AccountLog::TRANSACTIONIN_SELLER, '挂卖' . $currency_matches->symbol . '匹配成功,扣除卖方' . $currency_matches->currency_name);
            if ($result !== true) {
                throw new \Exception($result);
            }

            //增加买方的交易币
            $result = change_wallet_balance($to_currency, 2, $num, AccountLog::TRANSACTIONIN_REDUCE_ADD, $currency_matches->symbol . '买入被匹配,增加买方' . $currency_matches->currency_name);
            if ($result !== true) {
                throw new \Exception($result);
            }
            ...
            //扣除买方的冻结法币钱包的交易币
            $result = change_wallet_balance($to_legal, 2, -$cny, AccountLog::TRANSACTIONIN_REDUCE, $currency_matches->symbol . '买入被匹配:扣除买方已冻结' . $currency_matches->legal_name, true);
            if ($result !== true) {
                throw new \Exception($result);
            }

            //增加卖方法币钱包的交易币
            $result = change_wallet_balance($out_legal, 2, $cny, AccountLog::TRANSACTIONIN_SELLER_ADD, '挂卖' . $currency_matches->symbol . '匹配成功,增加卖方' . $currency_matches->legal_name);
            if ($result !== true) {
                throw new \Exception($result);
            }
