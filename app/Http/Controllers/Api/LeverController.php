...
use App\Models\{AccountLog, Currency, CurrencyMatch, CurrencyQuotation, LeverTransaction, LeverMultiple, Setting, TransactionComplete, TransactionIn, TransactionOut, Users, UsersWallet};
...
class LeverController extends Controller
{
    ...
    public function submit()
    {
        ...
        $password = Input::get('password', ''); //支付密码
        $now = time();
        $user_lever = 0;
    ...
    public function cancelTrade(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($user_id, $id) {
                ...
                if ($result !== true) {
                    throw new \Exception('撤单失败:' . $result);
                }
                $result = change_wallet_balance(
                    $legal_wallet,
                    3,
