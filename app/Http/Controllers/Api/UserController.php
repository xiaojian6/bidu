...
use App\Models\{AppApi, UserChat, Users, UserReal, Token, AccountLog, UsersWallet, UsersWalletcopy, Currency, InviteBg, Setting, UserCashInfo, ExchangeShiftTo,Optional};
use App\DAO\RPC;
...
class UserController extends Controller
{
    ...
    public function transtate_history(Request $request)
    {

        $limit = Input::get('limit', '10');
        $page = Input::get('page', '1');
        $currency = $request->get('currency'); //开户行
        ...
        $data['today_all_num']=AccountLog::where("user_id",$user_id)->where("type","308")->where("created_time",">",$lingchen)->sum("value");
        $data['all_num']=AccountLog::where("user_id",$user_id)->where("type","308")->sum("value");
        $data['list']=AccountLog::where("user_id",$user_id)->where("type","308")->orderBy("created_time","desc")->paginate($limit)->items();
