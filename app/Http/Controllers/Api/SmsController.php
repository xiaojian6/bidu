...
//        }
        //判断是邮箱还是手机
        $is_eamil = $this->check_email($mobile);
        if ($is_eamil) {
            $email = $mobile;
            if (empty($email)) {
            ...
            if ($scene == 'login' || $scene == 'change_password' || $scene == 'reset_password' || $scene == 'tibi' || $scene == 'band') {
                if (empty($user)) {
            ...
            try {
                ...
                $mail->setFrom($username, "[趣币]");//设置邮件来源  //发件人
    ...
    public function sendMail(Request $request)
    {
        ...
        else if ($scene == 'login' || $scene == 'change_password' || $scene == 'reset_password' || $scene == 'tibi' || $scene == 'band') {
            if (empty($user)) {
                return $this->error('账号错误');
            }
        } else {
