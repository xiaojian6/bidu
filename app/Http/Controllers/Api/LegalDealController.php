    ...
    public function postSend(Request $request)
    {
        ...
        try {
            ...
            if ($type == 'sell') {   //如果商家发布出售信息
                ...
                $seller->seller_balance = bc_sub($seller->seller_balance, $should_deduct_number, 5); //多扣一些手续费
                $seller->lock_seller_balance = bc_add($seller->lock_seller_balance, $total_number, 5); //手续费不计入冻结
                $seller->save();
                AccountLog::insertLog([
                    'user_id' => $user_id,
                    'value' => -$total_number,
                    'info' => '商家发布法币出售',
                    'type' => AccountLog::LEGAL_DEAL_SEND_SELL,
                ...
                AccountLog::insertLog([
                    ...
                    'value' => -$fee,
                    'info' => '商家发布法币出售,扣除手续费',
                    'type' => AccountLog::LEGAL_TRADE_FREE,
                    'currency' => $currency_id
                ]);
            }
    ...
    public function doDeal(Request $request)
    {
        ...
        try {
            ...
            if ($legal_deal_send->type == 'buy') { //求购
                ...
                $users_wallet->legal_balance = bc_sub($users_wallet->legal_balance, $number, 5); //手续费不计入冻结
                ...
                if (bc_comp($fee, '0') > 0) {
                    ...
                    AccountLog::insertLog([
                        ...
                        'before' => $users_wallet->legal_balance,
                        'change' => -$fee,
                        'after' => bc_sub($users_wallet->legal_balance, $fee, 5),
                    ]);
                    $users_wallet->legal_balance = bc_sub($users_wallet->legal_balance, $fee, 5);
                    $users_wallet->save();
                }
    ...
    public function backSend(Request $request)
    {
        ...
        try {
            ...
            if ($legal_send->type == 'sell') {   //如果商家发布出售信息
                ...
                //$seller->seller_balance += $legal_send->surplus_number;
                $refund = bc_add($legal_send->surplus_number, $legal_send->out_fee); //撤销时,连同手续费一同撤回
                $seller->seller_balance = bc_add($seller->seller_balance, $refund, 5);
                //$seller->lock_seller_balance -= $legal_send->surplus_number;
                $seller->lock_seller_balance = bc_sub($seller->lock_seller_balance, $legal_send->surplus_number, 5);
                $seller->save();
                AccountLog::insertLog([
                    'user_id' => $user_id,
                    'value' => $legal_send->surplus_number,
                    'info' => '商家撤回发布法币出售',
                    'type' => AccountLog::LEGAL_DEAL_BACK_SEND_SELL,
                    'currency' => $legal_send->currency_id
                ]);
                if (bc_comp($legal_send->out_fee, '0') > 0) {
                    AccountLog::insertLog([
                        'user_id' => $user_id,
                        'value' => $legal_send->out_fee,
                        'info' => '商家撤回发布法币出售,退回手续费',
                        'type' => AccountLog::LEGAL_CANCEL_TRADE_FREE,
                        'currency' => $legal_send->currency_id
                    ]);
                }
            }
...
//                $seller->seller_balance += $legal_send->total_number;
                $seller->seller_balance = bc_add($seller->seller_balance, $legal_send->surplus_number, 5);
//                $seller->lock_seller_balance -= $legal_send->total_number;
                $seller->lock_seller_balance = bc_sub($seller->lock_seller_balance, $legal_send->surplus_number, 5);

                $seller->save();
                AccountLog::insertLog(['user_id' => $user_id, 'value' => $legal_send->surplus_number, 'info' => '商家处理异常发布信息', 'type' => AccountLog::LEGAL_DEAL_ERROR_SEND_SELL, 'currency' => $legal_send->currency_id]);
            }
